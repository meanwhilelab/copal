...
- 🧭 **The Librarian reads your item's material for you.** A work item's **description** is your framing — the lens. From it, the Librarian compiles the item's **Context**: a chronological synthesis of everything linked to the item (what came first, what superseded what, what has sunk), recompiled automatically as links and descriptions change, and handed to any agent that resumes on the item via `get_context`. Optionally (`LINEAR_API_KEY`), every **Linear issue** an item tracks — any number of them, sub-issue trees included — joins the reading, kept current automatically.
- 🔗📤 **Share an item with anyone.** One revocable public link per item (`/s/<token>` — 256-bit token, hashed at rest, shown once) renders a read-only page with the item's identity, description, and Context — never its connections. Links unfurl richly in messaging apps, while the whole app stays **noindex** to search engines.
