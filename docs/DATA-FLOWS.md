...
| Linear issue identifiers (parsed from the canonical URL of each issue an item tracks) go out to Linear; each issue's title/description/state/updated-at — and its sub-issue tree, 3 levels deep — comes back, is **stored** in `contents.body`, and is folded into the text sent to the configured model chain | Linear (GraphQL API) → then the configured model chain | Live issue enrichment for item context synthesis | `POST /api/v1/items/:id/linear` (and the `link_linear_issue` MCP tool) at attach time, then the Housekeeper `item_context` job, which refreshes up to 10 tracked issues per compile — only when `LINEAR_API_KEY` is set |
...
- **Linear — GraphQL API (optional, `LINEAR_API_KEY`).** Purpose: fetch the live issue (title, description, state, updated-at) and its sub-issue tree for every Linear issue a board item tracks, so the item context compile reasons over current status rather than a URL. Only the parsed issue identifier is sent to Linear. **Issue text is stored at rest** in `contents` rows with `source_type='linear'` — a cache, refreshed on each `item_context` compile (up to 10 issues per compile, least-recently-refreshed first) and bounded at 3 levels deep, 25 children per level, 50 nodes and 12 000 characters per issue. Those rows are embedded like any other content, so tracked issues are searchable and can surface in resonance. Off by default — with the key unset, attaching still records the reference but nothing is fetched or stored. *Verify:* Linear's data-processing terms for API key access.
