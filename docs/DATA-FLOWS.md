...
| Linear issue identifier (parsed from the item's `link`) goes out to Linear; the issue's title/description/state/updated-at comes back and is folded into the text sent to the configured model chain | Linear (GraphQL API) → then the configured model chain | Live issue enrichment for item context synthesis | Housekeeper `item_context` job — only when `LINEAR_API_KEY` is set and the item's `link` is a Linear issue URL |
...
- **Linear — GraphQL API (optional, `LINEAR_API_KEY`).** Purpose: fetch the live issue (title, description, state, updated-at) for an item whose `link` points at a Linear issue, so the item context compile reasons over current status rather than the URL alone. Only the parsed issue identifier is sent to Linear; the returned fields then flow into the same model-chain call as the rest of the item's linked material (row above). Off by default — unset key = no calls, no behavior change. *Verify:* Linear's data-processing terms for API key access.
